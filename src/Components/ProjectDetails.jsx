...
export default function ProjectDetails({imgSrc,projectName,projectLink}) {
  ...
  return (
    <section className={`p-5 rounded-lg shadow-lg bg-white  md:w-[400px] w-full h-full outline outline-blue-400 sm:mt-5 ${!showDetails && 'md:h-[330px]'} `  }>
      ...
      <section className='flex justify-around items-center p-2  my-2'>
            ...
            <button className='p-4 rounded shadow bg-[#FFD700] text-black cursor-pointer font-bold text-base' onClick={()=>setShowDetails((old)=>!old)}>{showDetails ? "Hide ":"Show "}Details</button>            
      </section>
      {
                showDetails && <section>
